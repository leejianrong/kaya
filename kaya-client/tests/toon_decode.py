"""A TOON **decoder**, for tests only (KAN-541).

The shipped package only ever *writes* TOON (`kaya_client/toon.py`), so a decoder has no place in
the wheel or in V2a's release binary. But SLICES §V2a's contract is round-trip equality — "the
``toon`` output parses back to data **equal** to the ``json`` output", deliberately not a golden
string — and that needs something that reads TOON back. This is it.

It is written against the TOON grammar rather than against `kaya_client.toon`'s internals, so a bug
that made both agree would have to be made twice, in opposite directions. That is the whole value of
the file: a round-trip test against a decoder derived from the encoder proves the encoder is
self-consistent and nothing else.

Not a general-purpose parser. It assumes well-formed input (the encoder's output), 2-space
indentation and the comma delimiter, and it validates nothing it does not have to. **Do not promote
it out of ``tests/``** — the moment it ships, "encode-only" stops being true and the format grows a
second, unspecified consumer.

Ported alongside the encoder from pandan's V47 test decoder, for the reason `kaya_client.toon`'s
docstring gives: ADR 0003 keeps kaya's dependency on pandan one-way and soft, so a shared package is
...
def _split_cells(text: str, delimiter: str, *, braces: bool = False) -> list[str]:
    """Split on the delimiter, ignoring ones inside a quoted string — and, when ``braces`` is set,
    ones inside a ``{…}`` group. Row cells never contain a brace (the encoder quotes any value that
    does), but a *field list* nests them: ``ref,stored{ref,body},arg`` is three fields, not four."""
    cells: list[str] = []
    depth = 0
    start = 0
    i = 0
...
def _split_header(content: str) -> _Header | None:
    """Split ``key[N:]{a,b}: rest`` into its parts, or ``None`` when the line is not a header (an
    ordinary ``key: value`` pair, a list item, a bare scalar)."""
    pos = 0
    key: str | None = None
...
def _parse_list_item(lines: list[tuple[int, str]], i: int, indent: int) -> tuple[Any, int]:
    """A ``- …`` element. Its first entry rides the dash, so that entry's children sit two levels in
    while the element's remaining entries sit one level in."""
...
def decode(text: str) -> Any:
    ...
    if not lines:
        # An empty document is an empty object — TOON's one genuinely ambiguous encoding
        # (`encode({})` is the empty string), resolved the way the reference decoder resolves it.
        # No kaya endpoint returns a bare `{}`.
        return {}
