...
Now: **V3, the editor, is under way** — KAN-552 landed the skeleton and KAN-553 the editor, so
`frontend/` is a browsable three-region app with a router, a typed API layer, a credential seam and a
CodeMirror 6 pane that opens a note, edits it and saves it under ADR 0009's precondition, driven
against a real stack and a real PAT. What it does **not** have: the sidebar is a flat list rather than
a folder tree, there is no preview, no landing page, and a `409` is one honest line rather than a
banner. Three cards finish the slice and each replaces **one file**: KAN-554 (folder tree, real list,
live preview in `Sidebar.svelte`), KAN-555 (landing state and the PAT paste form), KAN-556 (the `409`
