...
list does. **KAN-962 closed the slice's last open bug**: a ranked result rendered in the folder tree
lost the ranking, and TREE is the default view, so the *default* rendering of a search was the one
that discarded relevance. A search now renders as the flat, relevance-ordered list whatever the
toggle was set to, the toggle leaves the screen while one is active rather than claiming a setting
that is being ignored, and one line says which ordering is on screen. What the slice's demo still
describes and the code does not do is *highlighting* the match in the browser. **`/links` and
...
nothing written for them. **KAN-568 put the browser half in**: the backlinks rail is `App.svelte`'s
fourth region, listing every note whose body links to the open one — driven against a real stack with a
real PAT, including the criterion most likely to be quietly wrong. Four notes linking to a target by
title kept every one of their backlinks across a rename of that target (`resolved_id`, not the string),
...
files behind it. KAN-571 then took ADR 0006 §3's free schema hygiene, and the honest proportion is in
that README beside it: the whole `tools/list` reply goes **948 → 785 tokens (−17.2%, `o200k_base`)**,
landing on the ADR's predicted ~16%, against the **84%** the same section measured for narrowing a
read — which these tools have had since KAN-569. Compaction is the small half by design, and ADR
0006's Finding 1 is that the resident surface is a ~4%-of-window line item beside a 22% one. What is *not* automated, and is worth knowing before trusting any of it, is the
...
**third** tier, the nearest `.mcp.json`, is **still deliberately not built, and V6 closing is what
settled that rather than what unblocked it** — the old wording here said it "arrives once V6 closes",
which the paragraph above has now made a stale conditional. Verified against the code on 2026-08-20:
`kaya-client/src/kaya_client/config.py` documents the tier as unbuilt and nothing under
`kaya-client/src/` or `mcp/src/` reads such a file. The reason is unchanged and does not depend on V6:
choosing which server entry in an MCP host's file is kaya's is a guess, and a host launching one
usually exports the `env` block anyway, so tier one covers the common case (see `config.py`). Also unbuilt: `make test-e2e` is still a stub, and **it is no longer blocked on any
...
**Two of KAN-568's properties are invisible to `flushSync`, and both were found by a green mutation**
(`frontend/src/components/BacklinksPanel.svelte`, `frontend/tests/backlinks-subject.test.ts`). Worth
knowing before writing the next async component in this SPA, because the trap is the instrument rather
than the code. `flushSync` runs the effects *and* the DOM update in one pass, so any property whose
whole content is "these two values are never observably out of step" is collapsed by the very thing
that would check it. Rendering the rail's subject off the `note` prop instead of off the `subject` rune
passed **all thirty** behavioural assertions and is still wrong — `note` moves one render earlier, so
the heading and the count name the new note over the old answer's rows, for one frame in a browser and
for none at all under `flushSync`. Dropping the `untrack` around the effect's read of that rune is the
same shape: the effect then depends on its own output, the extra run is one the identity guard returns
straight out of, and nothing observable happens. So both are asserted over the **parsed script**, in
`document-seam.test.ts`'s shape and for its stated reason. The third mutation was not structural and
became an ordinary test: aborting the request from the *guarded* effect's cleanup — the obvious place,
and where KAN-552 rehearsed the editor's teardown — also stayed green, because every assertion in the
file waited for the request to settle before touching the prop again. It kills a live request on the
re-run the guard exists to make a no-op, and the panel sits on `Loading…` for ever. **The lesson is the
window, not the three bugs**: a suite that only ever looks at settled states cannot see anything about
the unsettled one, and `tests/backlinks-panel.test.ts` now holds a response open with a pending promise
in four places for exactly that reason.
...
**A search is never rendered by the folder tree, and the toggle is off the screen while one is
active** (KAN-962, `frontend/src/components/Sidebar.svelte`, `frontend/README.md`'s last section).
This is the paragraph above applied: ordering and grouping records the caller already holds is
presentation, so the SPA gets to decide it — and the decision is that a ranked search and an arranged
corpus are two objects one toggle cannot mean both of. The tree groups by `path`, so it **cannot**
carry `ts_rank DESC, note.id DESC`: a folder exists because some note's path names it, folders sort
before leaves, and unpathed notes sit below the whole tree, so every order the server chose is
destroyed by the grouping. Because the tree is the *default* view, the default rendering of a search
was the one that discarded relevance — silently, after KAN-558 went to real trouble to make equal
ranks deterministic. Three things hold it together and each has a test in `tests/sidebar.test.ts`,
all of them over the rendered DOM. The **chosen** view is its own rune and a search never writes it,
so clearing a search restores the view a person picked; switching it for them is the card's option
(a), which looks identical on screen and never switches back. The **toggle is the other arm of the
`{#if}` that renders the notice**, so "a toggle reading `Tree` above a flat list" is unreachable
rather than untested — and it is hidden rather than disabled, because a greyed-out but still
highlighted `Tree` makes the same false claim. And **the notice says which ordering is on screen**
(*Ordered by relevance, not grouped by folder. The view toggle returns when you clear the search.*),
which is also the only thing that says where the toggle went, so it shows for a search that matched
nothing too. The order assertions open with a **positive control** — the same three notes rendered
through the tree, unsearched, come out in a different order — because an order test against a corpus
whose folder order happens to equal its rank order asserts nothing at all.
