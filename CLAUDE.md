...
Now: **V3, the editor, is under way** — KAN-552 landed the skeleton, KAN-553 the editor, KAN-555
the way in and KAN-556 the conflict banner, so `frontend/` is a browsable three-region app with a
router, a typed API layer, a credential seam, a real landing page that takes a **one-time PAT paste**
(and walks a `401` out rather than reloading through it), and a CodeMirror 6 pane that opens a note,
edits it, saves it under ADR 0009's precondition and offers **keep mine / keep theirs / side by
side** when that precondition fails — all driven against a real stack and a real PAT. What it does
**not** have: the sidebar is a flat list rather than a folder tree, and there is no preview. One card
finishes the slice: KAN-554 (folder tree, real list, live preview in `Sidebar.svelte`). After V3,
