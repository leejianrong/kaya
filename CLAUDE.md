...
| `mcp/` | KAN-569: a real server, not a skeleton — `src/kaya_mcp/server.py` registers ADR 0006's frozen six (`list_notes`, `get_note`, `create_note`, `edit_note`, `search_notes`, `get_backlinks`), each a thin call into `kaya-client`'s `render()`. Five work; `get_backlinks` refuses every call, naming KAN-566, because that capability has landed at no layer yet. The `MCP ⊆ CLI` direction and the rest of the honest state live in [`mcp/README.md`](mcp/README.md) — the one canonical place for it, per ADR 0006 §4 |
...
the documented tie-break, and refusing a present-but-blank `q`. **`--q` and the search box are in too** (KAN-559) — one
flag and one input, because the API returns the same `NoteList` a plain list does — and **KAN-962**
settled where a ranked search renders: the flat list, never the folder tree, whatever the sidebar's
toggle was set to. What the slice's demo still describes and the code does not do is *highlighting*
the match in the browser. **`/links` and `/backlinks` are in** (KAN-566): `GET
