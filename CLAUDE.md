...
Now: **V3, the editor, is nearly done** — KAN-552 landed the skeleton, KAN-553 the editor, KAN-555
the way in and KAN-554 the sidebar and the preview, so `frontend/` is a browsable app with a router, a
typed API layer, a credential seam, a real landing page that takes a **one-time PAT paste** (and walks
a `401` out rather than reloading through it), a CodeMirror 6 pane that opens a note, edits it and
saves it under ADR 0009's precondition, a **folder tree over the `path` column** beside the flat list,
and a **live preview** that follows the document without the editor's `$effect` re-running — all of it
driven against a real stack and a real PAT. What it does **not** have: a `409` is one honest line
rather than a banner. **KAN-556 is the last card in the slice** and it replaces one file,
`EditorPane.svelte`. After V3,
...
and say which. The current numbers and their breakdown are the table in `frontend/README.md`.
