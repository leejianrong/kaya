...
> **Provenance.** This document was written in the `pandan` repo at `docs/kaya-vision.md` and copied
> here on 2026-08-01 when kaya's own planning started. It is **settled input**, kept verbatim as the
> founding statement of intent. Where the plan has since departed from it, the plan wins and says so:
> [`PLAN.md`](./PLAN.md) is the live document, [`adr/`](./adr/) holds the decisions, and
> [`SLICES.md`](./SLICES.md) records the one deliberate reordering of §"First steps" below (fork F3).
> The pandan copy remains as that repo's pointer to its sibling.
...
> [pandan ADR 0018](https://github.com/leejianrong/pandan/blob/main/docs/adr/0018-pandan-rebrand.md).
> The board app is now **`pandan`**, this app is **`kaya`**
