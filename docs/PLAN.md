...
the drift (last synced 2026-09-25, KAN-1743, R19 shipped, R20 in progress).
...
| R20 | `kaya auth login/logout/status` (RFC 8628 device flow) + a hosted remote MCP endpoint (ADR 0013) | In progress — `kaya auth login/logout/me` **shipped** (KAN-1743: `/auth/device/*`, `device_authorization` table, `/device` consent page, `GET /api/v1/me`). Hosted MCP (KAN-1744) is **blocked on a design gap, not merely queued**: neither ADR 0013 nor pandan's own ADR 0025 specifies the browser-redirect OAuth 2.1 Authorization Code + PKCE mechanics a hosted MCP client needs — flagged to the maintainer rather than improvised past. Docs (KAN-1745) sequenced after it. |
...
| ~~**`render()` becomes a god function.**~~ **EXAMINED 2026-08-09 at V2b's close, and it did not.** ADR 0004 flagged this against itself and named V2b as the slice to judge it in, so here is the judgement. **`render()` has 26 lines of code across two functions and did not gain one of them in six cards** — the pipeline is still `project → truncate → attach_summary → serialize`, four statements under a type guard, with zero branches added. Its file grew by ~40 lines, all docstring. ADR 0004's stated mitigation is what held: each concern is a module with its own tests (`projection` 46 code lines, `truncation` 54, `aggregates` 22, `hints` 59, `overview` 18), so a concern arriving was a module arriving, not a branch. **Two structural facts did the work.** `render`'s signature is frozen by ADR 0005, so every card had to find its answer inside a step — the single-entity `--fields` refusal went to `payload.kind`, `--full` to `text_limit=0`, `help[]` to `(kind, noun)`, and KAN-549's row slice to a method on `Payload`. And `Payload`/`Shaped` are separate types, so ordering is enforced rather than remembered. **Where growth actually landed, for whoever watches next:** `serialization` is the largest module at 147 code lines (130 at V2a) and holds two enums, eight serializers and four layout helpers — it is what splits first if anything does. `_as_human` is the accumulation point to watch, now composing body + summary + hints, but it is four lines because each trailing block decides for itself whether it has anything to say and returns `None` if not. `Shaped` did **not** grow — still two fields — because hints derive from the payload at serialization time while `summary` must be attached state to reach `as_dict()`. That asymmetry is deliberate and is the one thing here a reviewer should expect to have explained to them. | Examined in **V2b** as planned. `render` is not the risk; `serialization` is the module to watch in V6. |
...
| **The MVP has no hosted deployment**, so nothing is proven under a real origin or real TLS until the homelab lands. **Confirmed and narrowed 2026-08-20 by KAN-722**, which asked the plain question — can somebody download `kaya-linux-x86_64` and use it? — and answered it with documentation rather than a deploy. The residue is three items and [ADR 0010 §Amendment (2026-08-20)](./adr/0010-no-hosted-deploy-until-the-homelab.md) is where they live; the "real cookie" this row used to name is not among them, because Q7 defers browser SSO on a fact about the Public Suffix List, so there is no cookie to fail to prove. | Reveals at the homelab, not in a slice. Accepted knowingly (ADR 0010); the manifests and the local cluster are the hedge. |
