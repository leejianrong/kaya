...
the drift (last synced 2026-09-25, KAN-1739, R19 in progress).
...
| R19 | Kaya becomes its own authorization server — no runtime dependency on pandan for identity (ADR 0012, supersedes ADR 0002) | In progress — KAN-1738/1739 shipped (GitHub OAuth App + fastapi-users + async engine + cookie sessions; `personal_access_token` table + `/api/v1/tokens` + the Tokens UI); KAN-1740/1741 (retiring introspection, the board-embed reconnect step) not started |
