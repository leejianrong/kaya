...
|------|-------------------------------|
| **Introspection latency is worse than tolerable.** A cache miss adds a full round trip to pandan, and a scale-to-zero backend can make that seconds, not milliseconds. The mitigation (cache, and a warm path) is designed in, but the number is unmeasured. | **V1** — it is an explicit acceptance criterion, measured on a cold and a warm upstream. |
...
| **Wikilink resolution costs a fan-out.** A note with forty `[[KAN-n]]` refs is forty card reads unless the resolver batches. Pandan has no batch card-read endpoint. | **V5** — if batching needs a pandan change, that's a second small ask on top of `GET /me`, and it should be found before the slice commits. |
