...
| Q8 | Per-note sharing or ACLs? | DECIDED (team-default), explicit share still DEFERRED | Owner-only remains true for an *explicit* per-note share — no decision to add one. But team-**default** access is now decided and building: a note with `team_id` set is visible to every member of that team by default, mirroring pandan ADR 0021's board model. Decided 2026-09-03, following the org/team spike (R15, KAN-1048) and pandan's Teams milestone shipping. See ADR 0011, `docs/roadmap/BREADBOARD.md` R16. | PLAN §Scope, ADR 0011 |
...
| Q44 | Now that identity is cut over (`KAN-1740`), what credential does the board-embed preview forward to pandan? | DECIDED | **Not the caller's own kaya-side bearer any more** — it stopped being a pandan credential the same cutover made. A caller explicitly connects a pandan PAT (`/api/v1/pandan-link`, `/pandan` SPA page), verified against pandan's own `GET /api/v1/me` before storage and encrypted at rest with `KAYA_AUTH_SECRET` (`Fernet`, not hashed — a linked token must be handed back to pandan raw on every render, unlike kaya's own PATs). `app/api/embeds.py` resolves the caller's kaya identity, looks up their link, forwards it. A caller with no link gets a distinct `not_connected` state, never conflated with `unavailable`. **`card_resolution.py`'s wikilink resolution has the identical defect and is deliberately left unfixed this round** — tracked here and in `docs/roadmap/BREADBOARD.md`'s R19 section, not silently carried forward. | ADR 0012's amendment, KAN-1741 |
| Q45 | Does ADR 0013 (or pandan's own ADR 0025, its stated mirror) specify the browser-redirect OAuth 2.1 Authorization Code + PKCE mechanics a hosted MCP client needs to obtain a token? | FORK | **No, and this is a genuine design gap, flagged rather than improvised past.** RFC 9728/7591/8707 (protected resource metadata, dynamic client registration, resource-indicator token binding) are all resource-*server*-side concerns — none of them says how a browser-based client (Claude.ai, ChatGPT, Cursor) actually authenticates a human and exchanges a code for a token. Pandan's own ADR 0024 explicitly deferred that flow's mechanics to ADR 0025 ("out of scope to build in this ADR's slice"); ADR 0025 then never supplied them either — it lists the same three RFCs and stops. Kaya's ADR 0013 inherits the identical gap by mirroring ADR 0025 verbatim. **`KAN-1743`'s RFC 8628 device flow does not have this problem** — it is a self-contained, redirect-free grant type, fully specified, and shipped. `KAN-1744` (hosted MCP) is blocked on this until a follow-up ADR (kaya's own, or adopting whatever pandan's own follow-up ADR settles if one lands) specifies the missing mechanics — not something this round's build should invent unilaterally. | ADR 0013, `docs/roadmap/BREADBOARD.md`'s R20 section, KAN-1744 |
...
| Q18 | Does a note ref survive export and re-import? | ASSUMED, now building | Yes. Export writes the `NOTE-n` ref into front matter; import re-uses it when free and records a remap when not. Export/import was out of the MVP but the ref was designed for it then because retrofitting identity is the expensive kind of change — **that build is R12, `docs/roadmap/BREADBOARD.md`**, and the assumption held: no identity retrofit needed. | ADR 0008 |
...
| ID | Question | Status | Answer or default | Landed |
|----|----------|--------|-------------------|--------|
| Q35 | Attachments and images? | DEFERRED, now building | Out for the MVP. Text-only markdown in Postgres. Object storage (R2) when genuinely needed, not before — **that "genuinely needed" point arrived; R14, `docs/roadmap/BREADBOARD.md`**. | PLAN §Scope |
| Q36 | A graph view? | DEFERRED, now shipped | Out for the MVP. `note_link` makes it possible later; building it now is decoration before the core works. **Shipped post-MVP — R10, KAN-1050.** | PLAN §Scope |
| Q37 | Embedding a live board view in a note? | DEFERRED, now shipped | Out of the MVP. It's the same read API as wikilink resolution, so it's cheap to add once ADR 0003's resolver exists. **Shipped post-MVP — R11, KAN-1049.** | PLAN §Scope |
...
| Security and secrets | Q5, Q6, Q9, Q33, Q40, Q42, Q44 · ADR 0002, ADR 0012 |
