...
| Q8 | Per-note sharing or ACLs? | DEFERRED, under investigation post-MVP | Owner-only for the MVP, mirroring pandan's pre-M5 stance. pandan's board membership model is the template when it's needed. **The org/team spike (R15, KAN-1048, `docs/roadmap/BREADBOARD.md`) is that "when needed" arriving** — no decision yet, investigation only. | PLAN §Scope |
