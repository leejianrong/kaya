...
drop this column, and ``tests/integration/test_alembic.py``
::``test_autogenerate_would_not_drop_anything`` is where that gets checked.
"""
...
revision never imports the models. The two copies are held honest by
``test_autogenerate_would_not_drop_anything``, which diffs the live schema against
``Base.metadata`` — a divergence between them *is* schema drift, and it reddens there.
"""
