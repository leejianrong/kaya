...
- **`failures.py`** holds [ADR 0005](../docs/adr/0005-born-agent-conformant.md) §contract 4's table —
  `0` ok · `1` runtime · `2` usage · `3` 401 · `4` 403 · `5` 404 · `6` 409 (`2` also covers `422`) —
  plus `EXIT_FOR_CODE`, keyed on
...
- **`2` is the caller's input being rejected — by argparse, or by the API.** `EXIT_FOR_STATUS` maps
  ...
  and everything the tables have no row for still exits `1`. KAN-839 put `422` in the same row: it
  is the same event by construction, because `backend/app/api/errors.py::handle_validation_error`
  is the only place a `422` originates and it fires only on a request body failing schema
  validation.
- **`6` is `409`: the note moved under a guarded write.** KAN-724's addition, and the first number
  ...
  code needs no edit. Pandan's CLI still maps its own `409`s to `1`; its matching row is tracked as
  KAN-831.
...
enum (2), missing token (1), 400 (2), 404 (5), 401 (3), 403 (4), 409 (6), 422 (2) — each asserting
stream, shape and exit code together, against an `httpx.MockTransport`. KAN-542 could only assert
them at the seam, because it had no verbs to produce one. Nine, not the six SLICES planned: the `400`
is KAN-718's, the `409` is KAN-724's and the `422` is KAN-839's. The `409`'s argv is the only one that
had to be a *particular* command — `note edit … --if-updated-at` — because it is the only refusal
reachable solely through a verb that sends a precondition; the `422` test reuses that same command to
reproduce a malformed `--if-updated-at` value, the case that surfaced the card.
